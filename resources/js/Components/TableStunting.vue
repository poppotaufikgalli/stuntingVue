<script setup>
	...
	const columnHead = computed({
	    get() {
	        return props.head && (props.head).length > 0 ? props.head : _.keys(props.data.data[0]);
	    },
	});
...
<template>
	...
	<div class="px-6 py-4 grid gap-3 md:flex md:justify-between md:items-center border-t border-gray-200 dark:border-gray-700">
		<div>
			...
			{{data.from}} Sampai {{data.to}} Dari {{data.total}} Data </span>
			</p>
		</div>
		<div>
