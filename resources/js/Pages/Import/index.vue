...
function hapusFile(item)
{
	if(item.row_count == 0){
		swal({
	        title: 'Yakin untuk Menghapus Data?',
	        text: "File Import "+ item.filename,
	        icon: 'warning',
	        ...
	        if (result.isConfirmed) {
	            ...
	            form.delete(route('import.destroy', {id: item.encid}))
	        }  
	    })	
	}else{
		usePage().props.flash = {
	        type: 'error',
	        message: 'File tidak bisa dihapus. Jumlah baris yang diimport : '+item.row_count,
	    }
	}
}
</script>
			          			...
			          			<table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
			            			...
			            			<tbody v-if="data" class="divide-y divide-gray-200 dark:divide-gray-700">
			              				<tr v-for="(item, key) in data.data" :key="key">
			                				...
			                				<td class="h-px w-72 whitespace-nowrap">
			                  					<div class="px-6 py-3">
			                    					<span class="block text-sm text-gray-500">{{item.row_count}}</span>
			                  					</div>
			                				</td>
			                				<td class="h-px w-px whitespace-nowrap">
			                  					<div class="px-6 py-3">
